...
function VocabSetsPage() {
  ...
  const empty = { title: "", description: "", color: colorChoices[0].value, wordIds: [] as string[] };
  ...
  const submit = (e: React.FormEvent) => {
    ...
    toast.success(`Đã tạo bộ "${form.title.trim()}"`);
    setForm(empty);
    setOpen(false);
  };
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-medium text-slate-600">Chọn từ ({form.wordIds.length})</label>
                  </div>
                  <div className="mt-2 max-h-48 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100">
                    {allWords.map((w) => {
                      ...
                      return (
                        <label key={w.id} className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-slate-50">
                          <input type="checkbox" checked={checked} onChange={() => toggleWord(w.id)} className="size-4 accent-green-500" />
                ...
                <DialogFooter>
                  ...
                  <button type="submit" className="px-4 py-2 rounded-2xl text-sm font-bold bg-green-500 hover:bg-green-600 text-white inline-flex items-center gap-2">
            ...
            <article key={s.id} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition flex flex-col">
              ...
              <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={() => handleDelete(s.id, s.title)} className="size-9 rounded-xl bg-slate-50 hover:bg-red-100 hover:text-red-600 text-slate-500 flex items-center justify-center transition" aria-label="Xoá">
